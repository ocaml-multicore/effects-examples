...
let rec alice_turn n = if n == 0 then Bob else bob_turn (n - move Alice n)
and bob_turn n = if n == 0 then Alice else alice_turn (n - move Bob n)
...
let valid_moves n = List.filter (fun m -> m <= n) [ 1; 2; 3 ]
...
let bf p n k =
  ...
  | Some i -> continue k (i + 1)
(* Among the winners, play the winning strategy (indices are zero-based) *)

(* Some example strategy handlers *)
let naive = strategy (fun _ -> ns)
let perfect = strategy (fun _ -> ps)
let bruteforce_bob = strategy (function Alice -> ps | Bob -> bf Bob)
...
type gametree = Winner of player | Take of player * (int * gametree) list

(* String representation of a gametree *)
let rec string_of_gametree : gametree -> string = function
  | Winner p -> "Winner(" ^ string_of_player p ^ ")"
  | Take (p, ts) ->
      "Take"
      ^ string_of_pair string_of_player
          (string_of_list (string_of_pair string_of_int string_of_gametree))
          (p, ts)

and string_of_pair :
      'a 'b. ('a -> string) -> ('b -> string) -> 'a * 'b -> string =
 fun string_of_x string_of_y (x, y) ->
  "(" ^ string_of_x x ^ ", " ^ string_of_y y ^ ")"

and string_of_list string_of_x xs =
...
let bob_cheats = strategy (function Alice -> ps | Bob -> cs)

(* The cheat detection mechanism *)
let check_move p n k =
  let m = move p n in
  if m < 1 || 3 < m then
    cheat p m (* player p cheats by making an illegal move m (m < 1 or 3 < m) *)
  else continue k m
  ...
  try_with m ()
    {
      effc =
        (fun (type a) (e : a Effect.t) ->
          match e with
          | Move (p, n) ->
              Some (fun (k : (a, _) continuation) -> check_move p n k)
          | _ -> None);
    }
...
let cheat_report m =
  ...
  with Cheat (p, n) ->
    failwith
      ("Cheater: " ^ string_of_player p ^ " took " ^ string_of_int n
     ^ " sticks!")

(* Another way to deal with cheaters is to disqualify them *)
let cheat_lose m =
  try m () with Cheat (Alice, _) -> Bob | Cheat (Bob, _) -> Alice
          ...
          | Choose ->
              Some
                (fun (k : (a, _) continuation) ->
                  continue k (Random.float 1.0 > 0.5))
          | _ -> None);
    }

let bob_maybe_cheats m =
  let h =
    if choose () then strategy (fun _ -> ps)
    else strategy (function Alice -> ps | Bob -> cs)
  in
  h m
      ...
      match_with f ()
        {
          ...
          effc =
            (fun (type b) (e : b Effect.t) ->
              ...
              | Get ->
                  Some
                    (fun (k : (b, t -> t * a) continuation) (s : t) ->
                      continue k s s)
              | Put s' -> Some (fun k _s -> continue k () s')
              | e -> None);
        }
    in
    snd @@ comp init
...
(* Update scoreboard *)
let increment_wins p =
  List.map (fun (p', n) -> if p == p' then (p', n + 1) else (p', n))

(* Post-processing handler that updates the scoreboard *)
let score_updater m = match m () with p -> put (increment_wins p (get ()))

(* Print the scoreboard *)
let print_board s =
  let rec make_whitespace n =
    if n > 0 then " " ^ make_whitespace (n - 1) else ""
  in
  let s =
    List.map
      (fun (p, n) ->
        ...
        "| " ^ player
        ^ make_whitespace (11 - String.length player)
        ^ "|"
        ^ make_whitespace (8 - String.length wins)
        ^ wins ^ " |")
      (List.sort
         (fun x y ->
           let n, n' = (snd x, snd y) in
           if n < n' then 1 else if n > n' then -1 else 0)
         s)
  in
  print_endline "/======================\\";
...
let printer m = match m () with _ -> print_board (get ())

(* Replays a game after n times *)
let rec replay n m = match m () with _ when n > 0 -> replay (n - 1) m | x -> x
...
let run_examples () =
  ...
  print_endline
    ">> game 7 |> (state s0) -<- printer -<- (replay 10) -<- coin -<- \
     score_updater -<- bob_maybe_cheats :";
  let _ =
    game 7
    |> state s0 -<- printer -<- replay 10 -<- coin -<- score_updater
       -<- bob_maybe_cheats
  in
