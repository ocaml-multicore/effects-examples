...
let (Node
      ( Node (Node (Empty, 1, 102, Empty), 5, 105, Empty),
        7,
        107,
        Node (Empty, 9, 109, Empty) )) =
  try update1 1 succ tree1 with NotFound -> insert 1 100 tree1
...
let (Node
      ( Node (Node (Node (Empty, 0, 100, Empty), 1, 101, Empty), 5, 105, Empty),
        ...
        107,
        Node (Empty, 9, 109, Empty) )) =
  try update1 0 succ tree1 with NotFound -> insert 0 100 tree1

(* The same as update1, but using Delimcc *)

let rec update2 :
    ('k, 'v) tree option prompt ->
    'k ->
    ('v -> 'v) ->
    ('k, 'v) tree ->
    ('k, 'v) tree =
...
let (Node
      ( Node (Node (Node (Empty, 0, 100, Empty), 1, 101, Empty), 5, 105, Empty),
        ...
        Node (Empty, 9, 109, Empty) )) =
  let pnf = new_prompt () in
  match push_prompt pnf (fun () -> Some (update2 pnf 0 succ tree1)) with
  | Some tree -> tree
  | None -> insert 0 100 tree1

(* Resumable exceptions *)
(* upd_handle is very problematic! *)
let upd_handle k = raise NotFound
...
let rec update4 :
    ('k, 'v) res prompt -> 'k -> ('v -> 'v) -> ('k, 'v) tree -> ('k, 'v) tree =
 fun pnf k f ->
  let rec loop = function
...
let (Node
  ...
  | Done tree -> tree
  | ReqNF (k, c) -> rebalance (match push_subcont c 100 with Done x -> x)
...
let (Node
      ( Node (Node (Node (Empty, 0, 100, Empty), 1, 101, Empty), 5, 105, Empty),
        ...
        Node (Empty, 9, 109, Empty) )) =
  let pnf = new_prompt () in
  match push_prompt pnf (fun () -> Done (update4 pnf 0 succ tree1)) with
  | Done tree -> tree
  | ReqNF (k, c) -> rebalance (match push_subcont c 100 with Done x -> x)
...
(* Several exceptions *)
let Empty =
  try try update1 7 upd_fun tree1 with NotFound -> insert 7 100 tree1
  with TooBig -> Empty

let Empty =
  try
    let pnf = new_prompt () in
    match push_prompt pnf (fun () -> Done (update4 pnf 7 upd_fun tree1)) with
    | Done tree -> tree
    | ReqNF (k, c) -> rebalance (match push_subcont c 100 with Done x -> x)
  with TooBig -> Empty
