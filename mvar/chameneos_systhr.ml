...
module Chameneos = struct
  ...
  let create =
    ...
    let new_id () =
      ...
      r
    in
    fun color -> { id = new_id (); color; meetings = 0; meetings_with_self = 0 }
  ...
  let run t place =
    let rec loop () =
      ...
      | Some other ->
          ...
          if t.id = other.id then
            t.meetings_with_self <- t.meetings_with_self + 1;
...
let work colors n =
  ...
  List.iter colors ~f:(fun c -> printf " %s" (Color.to_string c));
  printf "\n";
  ...
  List.iter cs ~f:(fun c ->
      printf "%d" c.C.meetings;
      spell_int c.C.meetings_with_self;
      printf "\n";
