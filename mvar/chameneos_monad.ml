...
let arrive (mpv : mp MVar.t) (finish : (int * int) MVar.t) (ch : chameneos) =
  ...
  let rec go t b =
    MVar.take mpv >>= fun w ->
    match w with
    | Nobody 0 -> MVar.put mpv w >> MVar.put finish (t, b)
    | Nobody q ->
        yield >> MVar.put mpv (Somebody (q, ch, waker)) >> MVar.take waker
        >>= fun w' -> go (t + 1) @@ inc w' b
    | Somebody (q, ch', waker') ->
        yield
        ...
        let () = ch := c'' in
        let () = ch' := c'' in
        MVar.put waker' ch >> go (t + 1) @@ inc ch' b
  in
  go 0 0
...
let work colors n =
  ...
  iter_p (fun (fin, ch) -> fork (arrive mpv fin ch)) comb >> map_p MVar.take fs
  >>= fun ns ->
  let () =
    List.iter
      ~f:(fun (n, b) ->
